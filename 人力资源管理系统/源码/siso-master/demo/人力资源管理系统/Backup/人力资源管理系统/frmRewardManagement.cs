...
namespace 人力资源管理系统
{
    public partial class frmRewardManagement : Form
    {
        public frmRewardManagement()
        {
            InitializeComponent();
        }
        #region ********主程序
        private void frmRewardManagement_Load(object sender, EventArgs e)
        {
            FillData();
        }
        public void FillData()
        {
            ...
            conn.Open();
            SqlDataAdapter da = new SqlDataAdapter("select * from 奖惩表", conn);
        ...
        {
            FindReward FrmFindReward = new FindReward();
            FrmFindReward.Text = "查询奖惩信息";
            FrmFindReward.ShowDialog(this);
        }

        private void btnAdd_Click(object sender, EventArgs e)
        {
            AddReward FrmAddReward = new AddReward();
            FrmAddReward.Text = "添加奖惩记录";
            FrmAddReward.ShowDialog(this);
        }

        private void btnChange_Click(object sender, EventArgs e)
        {
            //ChangeReward FrmChangeReward = new ChangeReward();
            //FrmChangeReward.Show();
            //*******************************************************************************************************
            if (dataGridView1.SelectedRows.Count == 0) return;
            ChangeReward modify = new ChangeReward();
            modify.Text = "修改奖惩信息";
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("确认删除选中的" + ROW.ToString() + "条记录吗？", "请确认删除", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                {
                    SqlCommand comm = new SqlCommand("delete from 奖惩表 where 员工号=@员工号", conn);
        ...
        {
            SearchInReward FrmSearchInReward = new SearchInReward();
            FrmSearchInReward.ShowDialog(this);
        }
