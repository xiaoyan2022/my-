...
namespace 人力资源管理系统
{
    public partial class frmWagesManagement : Form
    {
        public frmWagesManagement()
        {
            InitializeComponent();
        }
        #region *********主程序
        private void frmWagesManagement_Load(object sender, EventArgs e)
        {
            FillData();
        }
        public void FillData()
        {
            ...
            conn.Open();
            SqlDataAdapter da = new SqlDataAdapter("select * from 工资表", conn);
        ...
        {
            FindWages FrmFindWages = new FindWages();
            FrmFindWages.Text = "查询工资信息";
            FrmFindWages.ShowDialog(this);
        }

        private void btnAdd_Click(object sender, EventArgs e)
        {
            AddWages FrmAddWages = new AddWages();
            FrmAddWages.Text = "添加工记录";
            FrmAddWages.ShowDialog(this);
        }

        private void btnChange_Click(object sender, EventArgs e)
        {
            //ChangeWages FrmChangeWages = new ChangeWages();
            //FrmChangeWages.Show();
            //*********************************************************************************************************
            if (dataGridView1.SelectedRows.Count == 0) return;
            ChangeWages modify = new ChangeWages();
            modify.Text = "修改工资信息";
            modify.流水ID = dataGridView1.SelectedRows[0].Cells["流水号"].Value.ToString();
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("确认删除选中的" + ROW.ToString() + "条记录吗？", "请确认删除", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                {
                    SqlCommand comm = new SqlCommand("delete from 工资表 where 流水号=@流水号", conn);
                    comm.Parameters.AddWithValue("@流水号", rows[i].Cells["流水号"].Value);
        ...
        {
            SearchInWages FrmSearchInWages = new SearchInWages();
            FrmSearchInWages.ShowDialog(this);
        }
