...
<div id="main">
  <p id="title"><c:if test="${status == 1}">我的帖子</c:if><c:if test="${status == 0}">商品订单</c:if></p>
  <div id="mession">
    <ul id="m_con">
      <c:forEach items="${good}" var="g">
        <li>
          <p><span class="wait">活动交流</span><span class="title">${g.name}</span></p>
          <p>帖子内容<span style="margin-left:30px;margin-right: 30px;">发布时间:${g.time}</span></p>
          <p><span class="des">${g.des}</span></p>
          <c:if test="${status == 1}">
            <button onclick="del(${g.gid})">删除帖子</button>
          </c:if>
        </li>
      </c:forEach>
    </ul>
...
<script>
  function del(gid) {
    window.location.href="del_good?gid="+gid
  }
</script>
