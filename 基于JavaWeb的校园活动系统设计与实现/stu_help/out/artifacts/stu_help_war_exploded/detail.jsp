...
<div id="main">
  <div id="detail">
    <p>该活动信息<span id="close">关闭</span></p>
    <table>
      <tr><td>活动标题</td><td>${d.title}</td></tr>
      <tr><td>活动描述</td><td>${d.des}</td></tr>
      <tr><td>发布者</td><td>${d.pub}</td></tr>
      <tr><td>发布时间</td><td>${d.time}</td></tr>
    </table>
    <button onclick="accept(${d.tid})">立即报名</button>
  </div>
  <p id="title">任务中心</p>
  <div id="mession">
    <div id="search">
      ...
      <form action="taskListByCondition" onsubmit="return false">
    ...
    <ul id="m_con">
     <c:forEach items="${task}" var="t">
       <li>
         <p><span class="wait">待解决</span><span class="des" onclick="detail(${t.tid}">${t.title}</span></p>
         <p>${t.pub}<span style="margin-left:30px;margin-right: 30px;">2022-12-07</span>悬赏：${t.reward}￥</p>
       </li>
     </c:forEach>
    </ul>
  </div>
...
<script>
  function search(){
    ...
    window.location.href="taskListByCondition?title="+title+"&college="+college
  }
  ...
  function accept(tid) {
    window.location.href="update_task?tid="+tid
  }
  function detail(tid) {
