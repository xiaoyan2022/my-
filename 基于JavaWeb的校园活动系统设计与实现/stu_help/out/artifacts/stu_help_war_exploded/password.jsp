  ...
  <div id="mession">
    <ul id="m_con">
      <form action="password" method="post" onsubmit="return check()">
        <table>
          <tr><td>原密码 <input type="text" name="pwd" id="pwd"></td><tr><td><span style="color: red">${msg}</span></td></tr></tr>
          <tr><td>新密码 <input type="password" name="newPwd" id="newPwd"></td><td><span id="Reg"></span></td></tr>
...
<script>
  function check() {
    ...
    if(newPwd ==''||pwd ==''){
      ...
      if(pwd == newPwd){
        document.getElementById("Reg").innerHTML="新旧密码不能相同"
  ...
  function reg() {
    ...
    const result = patt.test(newPwd.trim())

    if(!result){
      document.getElementById("Reg").innerHTML=""
    }else{
      document.getElementById("Reg").innerHTML="密码不能纯数字"
      document.getElementById("Reg").style.color='red'
    }
