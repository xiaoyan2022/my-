...
<div id="main">
  <p id="title"><c:if test="${status == 1}">已发布任务</c:if><c:if test="${status == 0}">已接受任务</c:if></p>
  <div id="mession">
    <ul id="m_con">
      <c:forEach items="${task}" var="t">
        <li>
          <p><span class="wait"><c:if test="${status==1}">活动未开始</c:if><c:if test="${status==0}">活动已开始</c:if></span></p>
          <p><span class="title">${t.title}</span></p>
          <p>活动描述<span style="margin-left:30px;margin-right: 30px;"><span style="margin-right: 30px">开始时间 : ${t.time}</span>结束时间 : ${t.end}</span>参与人数：${t.reward}</p>
          <p><span class="des">${t.des}</span></p>
          <c:if test="${status == 1}">
            <button onclick="del(${t.tid})">取消活动</button>
          </c:if>
        </li>
      </c:forEach>
    </ul>
...
<script>
  function del(tid) {
    window.location.href="del_task?tid="+tid
  }
</script>
