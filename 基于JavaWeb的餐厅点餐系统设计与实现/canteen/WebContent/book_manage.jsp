...
<div id="zhu">
	...
	<tr><td>菜品图片</td><td>菜品名称</td><td>菜品类型</td><td>菜品口味</td><td>菜品价格</td><td>操作</td></tr>
	<c:forEach items="${books }" var="b">
	<tr>
	<td><img src="upload/${b.imgUrl }"></td>
	<td>${b.name }</td>
	<td>${b.author }</td>
	<td>${b.publish }</td>
	<td>${b.price }</td>
	<td><a href="javascript:;" onclick="del(${b.id })" id="add">下架</a><a href="bookListById?id=${b.id }" id="add">修改</a></td>
	</tr>
	</c:forEach>
	</table>
...
<%
 ...
 else if("添加成功".equals(msg)||"下架成功".equals(msg)||"修改成功".equals(msg)){%>
...
function del(id){
	if(confirm("确定要下架吗？")){
		window.location.href="del_book?id="+id
	}
}
